...
let out
try {
stiker = await sticker(img, false, global.igfg, global.author)
} catch (e) {
console.error(e)
} finally {
if (!stiker) {
...
stiker = await sticker(false, out, global.fgif, global.author)
}}
} else if (args[0]) {
if (isUrl(args[0])) stiker = await sticker(false, args[0], global.packname, global.author)

else return conn.reply(m.chat, `[❗] sorry but it was not possible to be the sticker with that link, please verify that the link is from an image`, fkontak, m)
